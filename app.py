...
class MerkleHellmanApp:
    ...
    def check_keys(self):
        try:
            ...
            w, q, r = private_key
            if not is_superincreasing_sequence(w):
                messagebox.showerror("Lỗi", "Dãy w không phải là dãy siêu tăng.")
                return
            if q <= sum(w):
                messagebox.showerror("Lỗi", "q phải lớn hơn tổng các phần tử trong w.")
                return
            if gcd(r, q) != 1:
                messagebox.showerror("Lỗi", "r và q không nguyên tố cùng nhau.")
                return
            messagebox.showinfo("Thành công", "Khóa riêng hợp lệ!")
